...
攻擊技能的射程必須在介面上看得到（追擊距離由天賦格會用到的技能射程決定，見下節）。
...
**追擊移動時計時器照走，但最多只累積到攻擊間隔的一半**（`CHASE_COOLDOWN_CARRY = 0.5`）。

三種處理各有問題，折半是唯一同時擋掉兩邊的：

| 做法 | 問題 |
|---|---|
| 追擊時歸零 | 冷卻是「距離上一次出手過了多久」，走路那段時間本來就過去了；抹掉它等於同一件事算兩次 |
| 走路時間全算 | 一刀一隻的密集怪群下，每次接敵冷卻都已補滿，兩次出手的間隔被走路時間吃掉一大截，**實際 DPS 高於攻速面板** |
| **累積到一半** | 走路不白走，也不會走到就打 |

**已經超過一半的不倒扣。**

三個分支（原地待命／站定等冷卻／追擊）現在是同一套規則的三種程度，
不再有「某一種情況特別吃虧」。
...
手動指定只覆蓋第 1 步，**目標死亡後直接落回第 3 步**（重選最近的一隻），
不維持空目標、不需要玩家取消。完整規格見 `03-combat.md` § 3.6.1。

手動指定**不改變**追擊距離、出手判定與 AoE 命中集合（§ 3.1／§ 3.4／§ 3.5）——
它只回答「主目標是誰」。
