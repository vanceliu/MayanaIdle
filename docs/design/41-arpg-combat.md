...
| 類型 | 距離 | 行為 |
|------|------|------|
| 近戰（`range` ≤ 1.5） | **相鄰格**（含斜角） | 角色走到目標旁邊才能攻擊 |
| 遠程（`range` > 1.5） | 真實座標距離 ≤ `range` | 只要視線無阻擋即可攻擊，產生投射物 |
| 自身 | 0（對自己） | 治癒/Buff 類技能，無需目標距離判定 |

**近戰判定以格為單位**：角色所在格與目標所在格相鄰（Chebyshev ≤ 1）即可出手，
不看真實座標的歐氏距離。射程 1.5 的技能（盾擊、裂傷斬、背刺、復仇之刃）走同一條規則。

理由是消除死角：角色停在格與格之間時真實距離可能是 1.52，剛好超出 1.5 打不到，
而所在格的格心明明相鄰 —— 尋路又找不出「該站哪一格」（自己已經站在那一格上），
於是不出手也不移動。判定改以格為單位後，「打得到」與「該站哪一格」變成同一個問題。

**射程 > 1.5 維持真實座標**：那個尺度沒有邊界縫隙問題，改判格座標會在遠距離累積誤差。
`range: 0` 是「同一點」，不是相鄰格。

出手判定與落腳格判定**共用 `isWithinAttackRange()`**（`systems/lineOfSight.ts`），
玩家與怪物同一條規則。兩邊各寫一份就會回到「一邊說已就位、一邊說打不到」的僵局。
...
- 用詞固定為**射程**。介面上的「範圍」已經被 AOE 半徑佔用（如「範圍(半徑3格/最多3隻)」），
  兩者不可混用
- **距離單位一律寫「格」**，1 格 ＝ 地圖 1 tile ＝ 座標系 1 單位。射程、AOE 半徑、
  天賦的半徑與走位距離走同一套座標，直接比大小，不做換算。**不可寫成「碼」**
...
- 近戰：尋路到目標相鄰格（§ 3.1 的相鄰判定）
- 遠程：找到距離 ≤ 技能 range 且有視線的位置（優先當前位置）
- 使用現有 A* pathfinding；避開佔位、落腳格判定等移動規則見 `38-map-control.md` § 38.5
...
- 多人時「玩家」為隊伍成員集合，怪物目標選擇見 `97-selfhosted-server.md` § 97.7.1
