...
> 狀態：**保留備查，不採用**。線上模式以 `97-selfhosted-server.md` 為準；本文件只有 § 2 tick 對齊表、§ 10 頻寬估算、§ 13 靜態模板被 97 引用。
> 文中的「單機版現況」寫於改造之前：持久層現已是 server 端 SQLite，client 沒有 IndexedDB。
...
> 狀態：**考慮中，未定案。** 與 § 2~§ 12 的全域 server authoritative 為互斥路線，擇一實作。

### 14.1 分區權責

| 區域 | 邏輯執行位置 | 一致性 |
|---|---|---|
| 村莊（交易／拍賣／公會／排行／信箱） | Server | 強一致，即時寫 PG |
| 野外（戰鬥／掉落／經驗／金幣） | Client 決定性模擬 | 每 5 分鐘同步一次 |

### 14.2 同步流程

```
1. 進野外前向 Server 申請 session
   Server 產生 seed，保存 seed + 起始狀態快照 + 起始 tick
2. Client 以該 seed 執行決定性模擬
3. 每 5 分鐘或強制同步事件觸發，Client 回報：輸入序列 + 結果摘要 + 結束 tick
4. Server 以同一份純函數重播該區段，比對後寫入 PG
```

規則：

- seed 由 Server 發放，Client 不得自選、不得重取
- 裝備 instance 由 Server 重播產生；Client 回報的 instance 不採信
- 全量驗證（非抽驗）：一段 5 分鐘無渲染模擬為毫秒級，500 人約 1.7 次/秒
- 同步只傳 delta（本區段新增／變更的 instance）；全量對帳僅在登入與回村執行

### 14.3 強制同步時機

回村 / 進入交易 / 排行結算 / 登出 / 副本進出。

### 14.4 決定性要求（硬性）

- 禁用 `Math.random()`，一律 seeded PRNG
- 禁止依賴 `Date.now()` 與幀率，時間一律以 tick 計數表示
- 數值運算使用整數或固定小數，不得依賴浮點跨環境一致
- 戰鬥邏輯為純函數，同一份程式碼可在 browser 與 Node 執行

此四點為 § 14 與 § 15 任一方案的共同前提，與 § 11 第 1~2 步同向。

### 14.5 已知限制

| 項目 | 規則 |
|---|---|
| 斷線 / 關閉分頁 | 未同步區段作廢，最多損失 5 分鐘進度 |
| 同帳號多裝置 | session 版本號樂觀鎖，衝突方該區段整段作廢 |
| 校驗失敗處理 | **未定** — 整段作廢或接受並記錄，需另行決定 |
| 排行榜 | 野外收益同步後才計分，排名延遲最多 5 分鐘 |
| 作弊防護上限 | 僅靠數值上限校驗不足（玩家可長期回報上限值）；防護強度來自 Server 發 seed + 重播 |

### 14.6 與 server authoritative 的資源比較（500 人）

| 項目 | 全域 authoritative（§ 2~§ 12） | 野外離線同步（§ 14） |
|---|---|---|
| 負載成長 | O(在線人數 × tick) | O(同步次數) |
| Server 端戰鬥計算 | 持續 | 每人每 5 分鐘一次重播 |
| 頻寬 | 1~2 KB/s/人 持續 | 每 5 分鐘一次批次 |
| Redis 事件排程 | 必要 | 不需要（野外部分） |
| 最壞進度損失 | 1~2 秒 | 5 分鐘 |
...
> 狀態：**組隊不在現行設計範圍內**（`17-mvp-priority.md` § 17.5 未列；`38-map-control.md` § 38.11 不做多人同地圖顯示）。本節僅為未來若導入時的架構選項，未定案。

### 15.1 衝突點

同場組隊與 § 14 互斥：決定性重播要求單一 client 產生完整輸入序列；兩個 client 各自模擬會發散，5 分鐘同步無法支撐隊友間的即時互動。

### 15.2 選項

| 選項 | 作法 | Server 成本 | 代價 |
|---|---|---|---|
| A. Host 制 | 隊長 client 跑唯一模擬，隊員送輸入給隊長，Server 仍發 seed 並重播驗證 | 只做訊息 relay | 隊長作弊影響隊員收益；隊長斷線整隊中斷；效能取決於隊長機器 |
| B. 組隊時 authoritative | 單人野外走 § 14，組隊隊伍放到 Server 跑 tick | 正比於同時組隊人數 | 兩條執行路徑，共用同一份純函數 |
| C. 非同步協作 | 公會加成／離線助戰，不做同場戰鬥 | 0 | 不提供同場組隊 |

### 15.3 不受影響的項目

交易／拍賣為村莊行為，走即時寫 PG 的強一致路徑，與 § 14 同步模型無衝突。
