...
對比靠稀缺才有用。放置遊戲一開好幾小時、畫面幾乎不變，**會變的只有事件**
（超重、瀕死、掉寶、升級、冷卻結束）—— 對比預算該幾乎全給它們。
框永遠都在，把預算花在框上等於把警報聲變成背景音。

因此兩層分開處理，方向相反：

| | 原則 | 理由 |
|---|---|---|
| **文字對比** | 要高，沒有取捨 | 不花任何視覺預算，低於 4.5:1 就是有人讀不到 |
| **框線對比** | 要克制 | 花掉的是事件的音量 |
...
- **字級一律 `var(--fs-xs)`，不可寫死 px**（否則文字放大時只有它不變，違反 § 34.6）
- 尺寸用 `em`，數字變兩位數時橫向長出去而不是擠成一團
- 外圈 `box-shadow` 一圈底色，與加粗後的框線隔開

**自動天賦只顯示「有沒有鑲入」的指示點，不顯示數量。**
...
| 戰鬥 | 戰鬥天賦格 |
| 補給 | 補給天賦格 |
| 天賦合成 | 天賦格與鑲材的合成（`51-auto-talent.md` § 51.5.2） |

| 項目 | 規則 |
|---|---|
| 天賦格 | 每格顯示 N 個條件槽 ＋ 1 個實作槽，**空槽必須可見**（要看得出還能鑲幾個） |
| 鑲材選單 | 只列出**已持有且未被佔用**的；未取得的走 Wiki（`43-wiki-system.md`） |
| 持有鑲材的清單 | 不在本面板，在背包的「天賦」分頁 |

### 背包

分頁列兩個：**一般**（現有背包格 ＋ 底部印記抽屜）、**天賦**（鑲材）。
