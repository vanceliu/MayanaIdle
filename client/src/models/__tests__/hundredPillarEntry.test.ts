 ...
 *
 * 改動前 `百柱塔 1F 通行卷軸` 是死資料：不在任何掉落表、非任何 `entryScrollName`、
...
describe('百柱塔 1F 通行卷軸', () => {
  ...
  it('是百柱塔 1~10F 的入場券', () => {
    expect(getRegion('hundred-pillar-1-10f')?.entryScrollName).toBe(SCROLL);
  });

  it('不再是死資料 —— 至少被一個 region 引用', () => {
    expect(REGIONS.filter(r => r.entryScrollName === SCROLL).length).toBe(1);
  });

  it('可反查回目的地 region，因此能放快捷鍵直飛', () => {
    expect(getEntryScrollRegion(SCROLL)).toBe('hundred-pillar-1-10f');
    expect(canQuickSlotItem('bag', SCROLL)).toBe(true);
  });
});
...
describe('百柱塔各段門禁完整性', () => {
  it('1~10F 到 91~100F 每一段都有各自的通行卷軸，且卷軸都存在於 seed', () => {
    ...
    const missing = segments.filter(r => !r.entryScrollName || !known.has(r.entryScrollName));
