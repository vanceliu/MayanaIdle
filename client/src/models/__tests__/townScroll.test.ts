...
describe('townScroll', () => {
  ...
  describe('findScrollInBag', () => {
    ...
    it('should find a neutral town scroll', () => {
      const bag = [{ name: '薄暮村回城卷軸', amount: 5 }];
    ...
    it('should find elsarth scroll', () => {
      const bag = [{ name: '艾爾薩斯回城卷軸', amount: 2 }];
      const result = findScrollInBag(bag);
      expect(result).not.toBeNull();
    ...
    it('should find varden scroll', () => {
      const bag = [{ name: '瓦爾登回城卷軸', amount: 1 }];
      const result = findScrollInBag(bag);
      expect(result).not.toBeNull();
    ...
    it('should return null when scroll amount is 0', () => {
      const bag = [{ name: '薄暮村回城卷軸', amount: 0 }];
      expect(findScrollInBag(bag)).toBeNull();
    });

    it('should return first available scroll when multiple exist', () => {
      const bag = [
        { name: '薄暮村回城卷軸', amount: 3 },
        { name: '艾爾薩斯回城卷軸', amount: 2 },
      ];
      const result = findScrollInBag(bag);
      expect(result).not.toBeNull();
      expect(result!.townId).toBe('neutral-town');
  ...
  describe('consumeTownScroll', () => {
    it('should decrement scroll amount by 1', () => {
      const bag = [
        { name: '薄暮村回城卷軸', type: 'scroll', amount: 5 },
        { name: '品質石', type: 'material', amount: 3 },
      ];
      const result = consumeTownScroll(bag, '薄暮村回城卷軸');
      expect(result.find(b => b.name === '薄暮村回城卷軸')?.amount).toBe(4);
      expect(result.find(b => b.name === '品質石')?.amount).toBe(3);
    });
    ...
    it('should remove scroll from bag when amount reaches 0', () => {
      ...
      const result = consumeTownScroll(bag, '薄暮村回城卷軸');
      expect(result.find(b => b.name === '薄暮村回城卷軸')).toBeUndefined();
      expect(result).toHaveLength(1);
    });

    it('should not affect other items', () => {
      const bag = [
        { name: '薄暮村回城卷軸', type: 'scroll', amount: 2 },
        { name: '艾爾薩斯回城卷軸', type: 'scroll', amount: 3 },
      ];
      const result = consumeTownScroll(bag, '薄暮村回城卷軸');
      expect(result.find(b => b.name === '薄暮村回城卷軸')?.amount).toBe(1);
      expect(result.find(b => b.name === '艾爾薩斯回城卷軸')?.amount).toBe(3);
    });
  });
});
