...
/** § 38.11.1 主導地形佔特色地形總量的下限 */
export const DOMINANT_MIN_FEATURE_SHARE = 0.50;
/** § 38.11.1 點綴地形各自佔特色地形總量的上限 */
export const ACCENT_MAX_FEATURE_SHARE = 0.20;
export const MIN_FEATURE_TYPES = 2;

/** § 38.12 叢聚分佈 */
export const CLUSTER_MIN_SIZE = 3;
export const CLUSTER_MAX_SIZE = 24;
...
/** § 38.12 硬性約束 5 */
export const DEAD_END_AREA_DIVISOR = 100;
/**
...
export const THEME_TERRAIN_PALETTES: Readonly<Record<MapTheme, readonly TileType[]>> = {
  ...
  'prison': [TileType.Wall, TileType.Water, TileType.Carpet, TileType.Pillar],
  'ancient': [TileType.Pillar, TileType.Wall, TileType.Carpet, TileType.Chasm],
  ...
  'tower': [TileType.Pillar, TileType.Carpet, TileType.Wall],
  'frost-tower': [TileType.Pillar, TileType.Water, TileType.Carpet, TileType.Wall],
  'lava-tower': [TileType.Lava, TileType.Pillar, TileType.Carpet, TileType.Wall],
};
...
export const MAP_DESIGN_PROFILES: Readonly<Record<string, MapDesignProfile>> = {
  ...
  'underwater-prison-3f': { archetype: 'room-corridor', density: 'standard', dominantTerrain: TileType.Carpet },
  ...
  'ancient-dungeon-9f': { archetype: 'room-corridor', density: 'sparse', dominantTerrain: TileType.Carpet, isBossFloor: true },
  // dragon
  ...
  'hundred-pillar-31-40f': { archetype: 'pillar-hall', density: 'standard', dominantTerrain: TileType.Carpet },
...
export function checkThemeTerrain(map: MapData, profile: MapDesignProfile): DesignViolation[] {
  ...
  if (profile.density === 'sparse') return [];
...
/** § 38.11.1 主導地形 ≥ 50%、點綴各 ≤ 20%、裝飾地面主導另有 35% 上限 */
export function checkDominantTerrain(map: MapData, profile: MapDesignProfile): DesignViolation[] {
  ...
  for (const tile of stats.featureTypes) {
    ...
    if (share > ACCENT_MAX_FEATURE_SHARE) {
      violations.push({
        rule: 'accent-terrain',
        message: `點綴地形 ${tile} 佔特色地形 ${(share * 100).toFixed(1)}%，`
          + `超過上限 ${ACCENT_MAX_FEATURE_SHARE * 100}%`,
      });
    }
  }
...
export function checkClustering(map: MapData, profile: MapDesignProfile): DesignViolation[] {
  ...
  for (const region of enforceSize ? regions : []) {
    if (region.length < CLUSTER_MIN_SIZE || region.length > CLUSTER_MAX_SIZE) {
      violations.push({
        rule: 'cluster-size',
        message: `障礙叢大小 ${region.length} 格，需在 ${CLUSTER_MIN_SIZE}~${CLUSTER_MAX_SIZE} 格之間`,
        positions: region,
      });
    }
  }
...
/**
 * 全部設計檢查，由便宜排到昂貴。
 * 最後兩項是全點對 BFS，成本遠高於前面的統計類檢查。
 */
const DESIGN_CHECKS: readonly ((map: MapData, profile: MapDesignProfile) => DesignViolation[])[] = [
  map => checkThemePalette(map),
  (map, profile) => checkWalkableRatio(map, profile),
  map => checkSpawnableRatio(map),
  (map, profile) => checkThemeTerrain(map, profile),
  (map, profile) => checkDominantTerrain(map, profile),
  map => checkCorridorWidth(map),
  map => checkSpawnClearance(map),
  map => checkDeadEnds(map),
  (map, profile) => checkClustering(map, profile),
  (map, profile) => checkOpenAreas(map, profile),
  map => checkLowObstacleBypass(map),
  map => checkDetourDistance(map),
];

/** 跑完所有設計檢查，回傳完整違規清單（空陣列 = 合規） */
export function validateMapDesign(map: MapData, profile: MapDesignProfile): DesignViolation[] {
  return DESIGN_CHECKS.flatMap(check => check(map, profile));
}

/**
 * 遇到第一項違規就停手，供生成腳本大量試錯時使用。
 * 語意與 `validateMapDesign` 一致（空陣列 = 合規），只是不列出後續違規。
 */
export function findFirstViolation(map: MapData, profile: MapDesignProfile): DesignViolation[] {
  for (const check of DESIGN_CHECKS) {
    const violations = check(map, profile);
