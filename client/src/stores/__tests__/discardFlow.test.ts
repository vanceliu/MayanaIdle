...
describe('丟棄流程', () => {
  beforeEach(() => {
    useGameStore.setState({
      bagItems: [
        { name: '紅色藥水', type: 'potion', amount: 10 },
  ...
  it('requestDiscard 只是掛起請求，不會立刻扣除', () => {
    useGameStore.getState().requestDiscard({ kind: 'bag', name: '紅色藥水', maxAmount: 10 });
  ...
  it('數量超過持有量時只丟到持有量為止，不會變負數', () => {
    useGameStore.getState().requestDiscard({ kind: 'bag', name: '銀礦石', maxAmount: 1 });
