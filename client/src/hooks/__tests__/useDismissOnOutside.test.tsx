...
describe('天賦選單點外面就關掉', () => {
  beforeEach(() => {
    ...
    useTalentStore.setState({ characterId: 1, slots: [slot] });
  });

  it('點空槽開選單，點外面關掉', () => {
    render(<TalentTypeEditor type="combat" />);
    fireEvent.click(screen.getAllByText('＋ 條件')[0]);
    expect(screen.getByText('生命與魔力')).toBeDefined();

    fireEvent.pointerDown(document.body);
    expect(screen.queryByText('生命與魔力')).toBeNull();
  });
  ...
  it('點選單自己不會關掉', () => {
    render(<TalentTypeEditor type="combat" />);
    fireEvent.click(screen.getAllByText('＋ 條件')[0]);
    fireEvent.pointerDown(screen.getByText('生命與魔力'));
    expect(screen.getByText('生命與魔力')).toBeDefined();
  });
});
