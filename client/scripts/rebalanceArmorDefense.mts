/**
 * 防具防禦重算 —— 依 `06-equipment-balance.md` § 6A.8.7 的「全身防禦目標」反推每件裝備的 defense。
 *
...
/** 防具與左手裝備沒有 T1：新手裝已經涵蓋那個量級，商店從 T2 開始賣（§ 6A.8.8） */
 ...
 * （§ 99.1 第 35 條仍允許，要恢復把 pos 調成 2 即可）。
 */
...
/** T1~T2 是過渡期：一律純防禦，不給額外屬性也不給回復（§ 6A.8.8） */
